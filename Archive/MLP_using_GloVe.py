...
emsize = 50
embedding_file = os.path.join(data_dir, f'glove.6B.{emsize}d.txt')
glove = dict(get_coefs(*o.rstrip().rsplit(' ')) for o in open(embedding_file))
weights_matrix = np.random.normal(scale=0.6, size=(vocab_size, emsize))
available_words = 0
unavailable_words = 0
...
model = TextClassificationModel(vocab_size, emsize, num_class, weights_matrix, trainable=True).to(device)
